...
def create_sub_vol_eval_dataset(annot_path, bifur_prob, end_prob, root_prob,
    ...
    if seq_range_half:
        seq_len = seq_len // 2
    annot_id = os.path.basename(annot_path).split('.')[0]
    ...
    num_rest_samples = num_samples_per_annot - \
        num_bifur_samples - num_end_samples - num_root_samples

    # Get bifucation and end points
    types = ['bifur', 'end', 'root', 'interm']
    types_to_point_ids = {'bifur': 'bifur_ids', 'end': 'endpts_ids',
                          'root': 'root_id', 'interm': 'interm_ids'}
    num_type_samples = [num_bifur_samples, num_end_samples,
                        num_root_samples, num_rest_samples]
    for t_i, type in enumerate(types):
        ...
        if type == 'interm':
            type_samples_idx = random.choices(
                range(total_type_points), k=num_type_samples[t_i])
        else:  # Use all other types of samples
            # type_samples_idx = random.sample(
            #     range(total_type_points), num_type_samples[t_i])
            type_samples_idx = list(range(total_type_points))
        ...
        for idx in type_samples_idx:
            ...
            if type == 'bifur':
                ...
                index = random.randint(
                    max(0, int(node_point_id) - seq_len), int(node_point_id))
            elif type == 'end':
                # sample a random index between (node_point_id - seq_len//2) and node_point_id - 1
                index = random.randint(
                    max(0, int(node_point_id) - seq_len), int(node_point_id) - 1)
            elif type == 'root':
                ...
                index = random.randint(int(node_point_id), min(
                    branch_length - 1, int(node_point_id) + seq_len))
            else:
